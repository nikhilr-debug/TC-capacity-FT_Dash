...
# Channel JSON Mapping Logic (Strictly overrides SQL)
CHANNEL_MAP = {
    ...
    for d in dates:
        ...
        for _ in range(50):
            ...
            data.append({
                ...
                "net_new_additions": random.randint(-15, 15) # Explicit net_new_additions
            })
...
tab1, tab2 = st.tabs(["📦 FT View", "⚡ TC Capacity"])
...
with tab2:
    ...
    # Strict Aggregation of net_new_additions
    metrics = [
    ...
    for col, (label, val) in zip([c1, c2, c3, c4, c5, c6], metrics):
        # Header Color Logic for Net Additions
        text_color = "var(--text)"
    ...
    # ── Custom Pandas Styler for TC Tables ──
    def style_tc_dataframe(dataframe, group_col):
        unique_groups = dataframe[group_col].unique()
        
        # Alternate Background Colors based on Group ID + Strict Red/Green Formatting for net additions
        def highlight_rows(row):
            if row["Week_start"] == "-": # Highlighting the TOTAL row
    ...
    with st.expander("🏆 Top N VLs Configurable View"):
        ...
        # Execute Top N Filtering & Channel Overrides
        tmp_tc = df_tc.copy()
        if tc_channels:
        ...
        # Enforce categorical sorting so historical weeks follow the master VL rank
        ...
        # Note: Do not append a TOTAL row to this view as it breaks categorical sorting
